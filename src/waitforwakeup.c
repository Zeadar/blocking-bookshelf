...
struct result wait_for_wakeup(void) {
    ...
    ret_val = sd_bus_add_match(bus, NULL,
                               "type='signal',"
    ...
    for (;;) {
        ...
        if (sd_bus_message_is_signal(message, "org.freedesktop.login1.Manager",
                                     "PrepareForSleep")) {
            ...
            if (sleeping ==
                0) { // PrepareForSleep returns "False" upon waking up
                break;
            }

        } else {
            sd_bus_message_unref(
                message); // Still unref even if message is not signal
        }
    }
